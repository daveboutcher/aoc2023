...
impl Map {
    ...
    fn clear(&mut self) {
        self.visited.clear();
        self.energized = vec![vec![false; self.cols()]; self.rows()];
    }
...
fn solve() -> usize {
    ...
    for row in 0..map.rows() {
        map.clear();
        map.do_beam(&Pos { row, col: 0 }, Dir::E);
        biggest = cmp::max(biggest, map.num_energized());
    ...
    for col in 0..map.cols() {
        map.clear();
        map.do_beam(&Pos { row: 0, col }, Dir::S);
        biggest = cmp::max(biggest, map.num_energized());
